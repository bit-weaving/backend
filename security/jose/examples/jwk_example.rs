use jose::jwk::{Jwk, JwkSet, KeyOperation, KeyType, PublicKeyUse};
...
fn main() {
    ...
    let rsa_jwk = Jwk::new(KeyType::Rsa)
    ...
    let ec_jwk = Jwk::new(KeyType::EllipticCurve)
    ...
    let symmetric_jwk = Jwk::new(KeyType::OctetSequence)
    ...
    let consistent_key = Jwk::new(KeyType::Rsa)
        .with_public_key_use(PublicKeyUse::Signature)
        .with_key_operations(vec![KeyOperation::Sign, KeyOperation::Verify]);

    println!(
        "Consistent key (sig use with sign/verify ops): {}",
        consistent_key.validate_key_usage_consistency()
    );

    let inconsistent_key = Jwk::new(KeyType::Rsa)
        .with_public_key_use(PublicKeyUse::Signature)
        .with_key_operations(vec![KeyOperation::Encrypt]);

    println!(
        "Inconsistent key (sig use with encrypt op): {}",
        inconsistent_key.validate_key_usage_consistency()
    );
}
